  ...
  return (
    <Card>
      ...
      <CardContent>
        <ChartContainer
          className="h-[260px]"
          config={{ value: { label: "Value", color: "hsl(var(--brand))" }, deposits: { label: "Deposits", color: "hsl(var(--muted-foreground))" } }}
        >
          <Recharts.AreaChart data={data}>
            <Recharts.CartesianGrid vertical={false} strokeDasharray="3 3" />
            <Recharts.XAxis dataKey="date" tickLine={false} axisLine={false} tickMargin={8} />
            <Recharts.YAxis tickLine={false} axisLine={false} tickMargin={8} />
            <ChartTooltip content={<ChartTooltipContent />} />
            <ChartLegend content={<ChartLegendContent />} />
            <Recharts.Area dataKey="value" type="monotone" stroke="hsl(var(--brand))" fill="hsl(var(--brand)/.25)" />
            <Recharts.Area dataKey="deposits" type="monotone" stroke="hsl(var(--muted-foreground))" fill="hsl(var(--muted-foreground)/.15)" />
          </Recharts.AreaChart>
        </ChartContainer>
      </CardContent>
    </Card>
